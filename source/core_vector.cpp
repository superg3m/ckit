...
internal void* MACRO_vector_insert_header(void* vector, VectorHeader header) {
    memory_copy(&header, vector, sizeof(header), sizeof(header));
...
internal inline void _vector_update_header(void* vector, VectorHeader newHeader) {
    u8* dest_ptr = memory_retreat_new_ptr(vector, sizeof(newHeader)); 
    memory_copy(&newHeader, dest_ptr, sizeof(newHeader), sizeof(newHeader));
}

internal inline VectorHeader _vector_extract_header(const void* vector) {
    VectorHeader header;
    u8* source_ptr = memory_retreat_new_ptr(vector, sizeof(header));
    memory_copy(source_ptr, &header, sizeof(header), sizeof(header));
    return header;
}
...
internal u32 _vector_new_allocation_size(VectorHeader* header) {\
    header->capacity *= 2;
...
internal void* _vector_grow(void* vector) {
    ...
    void* ret = memory_reallocate(vector, _vector_new_allocation_size(&header));
    _vector_insert_header(ret, header);
...
void* MACRO_vector_push(void* vector, const void* element) {
    ...
    _vector_update_header(vector, header);
    return vector;
}
