...
#include "../../Memory/ckit_memory.h" // Actually think about this maybe don't expose this to the user?
...
void* MACRO_vector_create(u32 size, u64 capacity, size_t type_size_in_bytes) {
    ...
    ckit_assert_msg(ret, "vector_create: failed to allocate - %d bytes", vector_allocation_size);
...
u64 vector_size(void* vector) {
	ckit_assert_msg(vector, "The dynamic array size failed (freed before this call!)\n");
    VectorHeader* header = _vector_extract_header(vector);
    return header->size;
}
...
void* MACRO_vector_push(void* vector, const void* element) {
	ckit_assert_msg(vector, "The vector push failed (freed before this call!)");
    VectorHeader* header = _vector_extract_header(vector);
...
void* MACRO_vector_pop(void* vector) {
	ckit_assert_msg(vector, "The vector pop failed (freed before this call!)\n");
    VectorHeader* header = _vector_extract_header(vector);
	ckit_assert_msg(header->size > 0, "The vector pop failed (no elements to pop!)\n");
	header->size--;
	return (u8*)vector + (header->size * header->type_size_in_bytes);
}

void* MACRO_vector_free(void* vector) {
	ckit_assert_msg(vector, "The vector free failed (freed before this call!)\n");
	VectorHeader header = *_vector_extract_header(vector);
	u32 vector_allocation_size = sizeof(header) + (header.type_size_in_bytes * header.capacity);
    vector = (u8*)vector - sizeof(header);
	memory_free(vector);
